    ...
    $tree.find("li")
        ...
        .find(":checkbox")
            .change(function() {
                ...
                var $this_single_checkbox_is_checked = jQuery(this).siblings(".single_checkbox").hasClass("checked");
                // Debug: console.info($all.length + " : " + $checked.length + " : " + $this_single_checkbox_is_checked);
                // All children are checked
                if ($all.length == $checked.length && $this_single_checkbox_is_checked) {
                	// Already checked
                ...
                else if($checked.length == 0 && !$this_single_checkbox_is_checked) {
                    jQuery(this)
                        //.attr("checked", "")
                        .siblings(".checkbox")
                            .removeClass("checked")
        ...
        .each(function() {
            ...
            if (jQuery(this).is(":has(ul)")) {
                ...
                $arrow.click(function() {
                    ...
                    if (jQuery(this).hasClass("collapsed")) {
                    	...
                    	jQuery(this).siblings("ul").css({overflow: 'hidden'});
                    	jQuery(this).siblings("ul").css({
                    		height: '0%',
                    		width: '100%'
                    	});
                    	...
                    	//jQuery(this).siblings("ul").css({height: 'block'});
            ...
            // The click event for the usual type of checktree checkbox (i.e. not a single_checkbox)
            $checkbox.click(function() {
                ...
                if (jQuery(this).hasClass("checked")) {
                   ...
                   // alert("before first click cal:  " + jQuery(this).siblings(".single_checkbox").not(".checked").length);
                    
                    // If the sibling single_checkbox isn't already checked, check it
                   // jQuery(this).siblings(".single_checkbox").not(".checked").click();
                    jQuery(this).siblings(".single_checkbox").not(".checked")
					.removeClass("half_checked")
					.addClass("checked")
                    ;
                    // Go to the children list, and find all unchecked checkbox images
                ...
                // If Unchecked:
                    ...
                    // If the sibling single_checkbox is checked, uncheck it
                    jQuery(this).siblings(".single_checkbox.checked").click();
                    ...
                    // Un half-check this parent (added by ndg, 1-17-11)
                    jQuery(this)
                        .removeClass("half_checked")
                        .removeClass("checked");
                }
                ...
                // Tell the sibling checkbox to update its status
               // jQuery(this).siblings(":checkbox").change();
            });
            ...
            $single_checkbox.click(function() {
                ...
                if (jQuery(this).hasClass("checked")) {
                    ...
                    // Not necessary, as the sc only relates to the sibling label, and not the sibling checkbox
                    /*
                    var $sibling_checkbox = jQuery(this).siblings(".checkbox");
                    
                    // Sibling is already checked, so don't need to do anything
                    if ($sibling_checkbox.hasClass("checked")) {
                    }
                    
                    // Check the status of children checkboxes
                    else {
                    	// There is at least one unchecked child, so don't need to do anything
                    	if (jQuery(this).siblings("ul").find(".checkbox:not(.checked)").length > 0) {
                    		
                    	}
                    	
                    	else {
							// There aren't any children, so check this checkbox
                    		if (jQuery(this).siblings("ul").find(".checkbox").length == 0) {
								$sibling_checkbox.addClass("checked");
							}
							
							// All the children are unchecked, so don't do anything
							else {
                ...
                // If Unchecked:
                    ...
                    // Check the status of children checkboxes
                    	...
                    	// There aren't any unchecked children, so uncheck the sibling checkbox
                    	if (jQuery(this).siblings("ul").find(".checked").length == 0) {
                    		$sibling_checkbox.removeClass("checked");
                    	}
                    	
                    	else {
                    		
                    		// There aren't any unchecked children, so check the sibling checkbox
                    		if (jQuery(this).siblings("ul").find(".checkbox:not(.checked))").length == 0) {
								$sibling_checkbox
									.removeClass("half_checked")
									.addClass("checked");
							}
                    		// There is at least one checked child, so half_check the sibling checkbox
                    		else {
                    			$sibling_checkbox
									.removeClass("checked")
									.addClass("half_checked");
